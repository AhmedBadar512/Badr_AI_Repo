...
class RMI(K.losses.Loss):
    def __init__(self, pool_stride=3, method=K.layers.MaxPool2D, rmi_radius=3, from_logits=False, **kwargs):
        ...
        self.pooling = method(pool_stride, pool_stride, padding="same")  # TODO: Add option to pool using bi-linear interpolation
    ...
    def call(self, y_true, y_pred):
        ...
        y_true_mean, y_pred_mean = tf.reduce_mean(y_true_vectors, axis=-1, keepdims=True), tf.reduce_mean(y_pred_vectors, axis=-1, keepdims=True)
        y_true_vectors, y_pred_vectors = y_true_vectors - y_true_mean, y_pred_vectors - y_pred_mean
        y_true_cov, y_pred_cov = tf.linalg.matmul(y_true_vectors, y_true_vectors, transpose_b=True), tf.linalg.matmul(y_pred_vectors, y_pred_vectors, transpose_b=True)
