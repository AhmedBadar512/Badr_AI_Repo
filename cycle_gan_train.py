...
args.add_argument("-s", "--save_dir", type=str, default="./cyclegan_runs", help="Save directory for models and tensorboard")
...
CROP_WIDTH = args.c_width if args.c_wifth < IMG_WIDTH else IMG_WIDTH
LAMBDA = 10
EPOCHS = args.epochs
...
logdir = "{}_{}_e{}_lr{}_{}x{}".format(time, MODEL, EPOCHS, LEARNING_RATE, IMG_HEIGHT, IMG_WIDTH)
...
def write_to_tensorboard(g_loss_g, g_loss_f, d_loss_x, d_loss_y, c_step, writer):
    with writer.as_default():
        ...
        if len(physical_devices) > 1:
            ...
            img_a = image_x
            img_b = image_y
        img_size_a, img_size_b = img_a.shape[1] * img_a.shape[2] * img_a.shape[3], img_b.shape[1] * img_b.shape[2] * img_b.shape[3]
        mean_a, mean_b = tf.reduce_mean(img_a, axis=[1, 2, 3], keepdims=True), tf.reduce_mean(img_b, axis=[1, 2, 3], keepdims=True)
        adjusted_std_a = tf.maximum(tf.math.reduce_std(img_a, axis=[1, 2, 3], keepdims=True), 1/tf.sqrt(img_size_a/1.0))
        adjusted_std_b = tf.maximum(tf.math.reduce_std(img_b, axis=[1, 2, 3], keepdims=True), 1/tf.sqrt(img_size_b/1.0))
...
def save_models():
    K.models.save_model(generator_g, os.path.join(save_dir, MODEL, str(epoch), "gen_g"))
    K.models.save_model(generator_f, os.path.join(save_dir, MODEL, str(epoch), "gen_f"))
    K.models.save_model(discriminator_x, os.path.join(save_dir, MODEL, str(epoch), "disc_x"))
    K.models.save_model(discriminator_y, os.path.join(save_dir, MODEL, str(epoch), "disc_y"))
...
for epoch in range(START_EPOCH, EPOCHS):
    ...
    for image_x, image_y in zip(train_A, train_B):
        ...
        n += 1
        if n % 20 == 0:
            write_to_tensorboard(gen_g_loss, gen_f_loss, disc_x_loss, disc_y_loss, (epoch * total_samples // BATCH_SIZE) + n, train_writer)
