...
args.add_argument("-d", "--dataset", type=str, default="cityscapes",
                  ...
                  choices=["cityscapes19", "cityscapes", "scurry"])
...
logdir = "{}_{}_e{}_lr{}_{}x{}_{}_{}".format(time, MODEL, EPOCHS, LEARNING_RATE, IMG_HEIGHT, IMG_WIDTH, gan_mode, dataset)
...
with mirrored_strategy.scope():
    ...
    segmentor = get_model("unet", classes=args.classes)
...
def write_to_tensorboard(g_adv_loss, g_kl_loss, g_vgg_loss, g_feautre_loss, disc_loss, c_step, writer):
    with writer.as_default():
        ...
        if len(physical_devices) > 1:
            img = tf.cast(tf.concat(mini_batch[0].values, axis=0), dtype=tf.float32) / 127.5 - 1
            if True:
                seg = segmentor(img)
            else:
                o_seg = tf.cast(tf.concat(mini_batch[1].values, axis=0), dtype=tf.int32)
                seg = tf.one_hot(o_seg[..., 0], args.classes)
        ...
        processed_labs = tf.cast(tf.squeeze(colorize(processed_labs)), dtype=tf.uint8) if cs_19 \
            else tf.cast(tf.squeeze(colorize(processed_labs, cmap)), dtype=tf.uint8)
...
def train_step(mini_batch, random_style=False, n_critic=5):
    ...
    with tf.GradientTape(persistent=True) as tape:
        ...
        seg = tf.nn.softmax(segmentor(img, training=True), axis=-1)
        # ============ Generator Cycle =============== #
        ...
        seg_loss = tf.reduce_mean(seg * tf.math.log(seg + 1e-8))

        total_gen_loss = g_adv_loss + g_kl_loss + g_vgg_loss + g_feautre_loss + seg_loss
    ...
    return g_adv_loss, g_kl_loss, g_vgg_loss, g_feautre_loss, disc_loss, seg_loss
...
def distributed_train_step(dist_mini_batch):
    pr_g_adv_losses, pr_g_kl_losses, pr_g_vgg_losses, pr_g_feautre_losses, pr_disc_losses, pr_seg_losses = \
    ...
    r_disc_loss = mirrored_strategy.reduce(tf.distribute.ReduceOp.MEAN,
                                           pr_disc_losses,
                                           axis=None)
    r_seg_loss = mirrored_strategy.reduce(tf.distribute.ReduceOp.MEAN,
                                           pr_seg_losses,
                                           axis=None)
    return r_g_adv_loss, r_g_kl_loss, r_vgg_loss, r_feature_loss, r_disc_loss, r_seg_loss
...
for epoch in range(START_EPOCH, EPOCHS):
    ...
    for mini_batch in processed_train:
        ...
        g_adv_loss, g_kl_loss, g_vgg_loss, g_feautre_loss, disc_loss, s_loss = distributed_train_step(mini_batch)
        print("Epoch {} \t Gen_Adv_Loss: {}, KL_Loss: {}, Gen_VGG_Loss: {}, Gen_Feature_Loss: {}, Disc_Loss: {}, S_Loss: {}, Memory_Usage:{}".format(
            epoch + 1, g_adv_loss, g_kl_loss, g_vgg_loss, g_feautre_loss, disc_loss, s_loss, psutil.virtual_memory().percent))
        n += 1
        with train_writer.as_default():
