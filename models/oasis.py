...
class OASISGenerator(K.Model):
    def __init__(self, channels_list=None, sn=True, init=K.initializers.VarianceScaling(scale=1e-3)):
        super().__init__()
...
class OASISDiscriminator(K.Model):
    def __init__(self, classes=19, channels_list=None, init=K.initializers.VarianceScaling(scale=1e-2)):
        super().__init__()
        if channels_list is None:
